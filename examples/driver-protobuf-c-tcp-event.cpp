...
static DriverThread::Thread mainThread;

static NTSTATUS
ProtobufProcessing(void) {
  NTSTATUS Status;
...
static NTSTATUS mainThreadRoutine(PVOID user)
{
  UNREFERENCED_PARAMETER(user);

  return ProtobufProcessing();
}
...
DriverEntry(_In_ PDRIVER_OBJECT DriverObject,
  ...
  return mainThread.Start(mainThreadRoutine, NULL);
}
VOID DriverUnload(_In_ struct _DRIVER_OBJECT *DriverObject) {
  UNREFERENCED_PARAMETER(DriverObject);

  mainThread.WaitForTermination();
  DebuggerPrint("Bye.\n");
}
}
